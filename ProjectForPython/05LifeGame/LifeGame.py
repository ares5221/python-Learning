...
class world(object):
    ...
    def __init__(self, scale=100):
        ...
        self.py = 5
        self.px = 5
        self.xlen = int(400 / self.scale)
        self.ylen = int(400 / self.scale)

        self.screen = pygame.display.set_mode(
            [2 * self.px + self.scale * self.xlen, 2 * self.py + self.scale * self.ylen])
    ...
    def displaycell(self, x, y):
        for i in range(10):
            print
            self.worldmap[x][y].nowstate, self.worldmap[x][y].nextstate
            self.updateworld()
            self.evolution()
            time.sleep(0.3)
        pygame.exit()
            ...
            for j in range(self.scale):
                if self.worldmap[i][j].nowstate == 1:
                    pygame.draw.rect(self.screen, white,
                                     [i * self.xlen + self.px, j * self.ylen + self.py, self.xlen, self.ylen])
